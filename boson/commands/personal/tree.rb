module Tree
  ...
  def inheritance_tree(klass, options={})
    render klass, {:class=>:parent_child_tree, :children_method=>:class_children, :type=>:directory}.merge(options)
  end
  ...
  def nested_tree(klass, options={})
    render klass, {:class=>:parent_child_tree, :children_method=>:nested_children, :value_method=>:nested_name,
